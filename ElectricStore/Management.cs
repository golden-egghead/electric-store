...
namespace PRN211PE_FA22_TrialTest_NguyenTrungTin
{
    public partial class Management : Form
    {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow selectedRow in dgvManagement.SelectedRows)
            {
                var productID = selectedRow.Cells["Id"].Value.ToString();

                var existedProduct = productServices.GetAll().FirstOrDefault(p => p.Id.ToString() == productID);

                if (existedProduct != null)
                {
                    productsToDelete.Add(existedProduct);
                }
            }
